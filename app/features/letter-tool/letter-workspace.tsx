  ...
  return (
    <div className="flex min-h-screen flex-col" dir={dir} data-hc={highContrast ? "true" : undefined} data-lt={largeText ? "true" : undefined}>
      ...
      <ProductHeader
        ...
        controls={
          <>
            <button onClick={toggleLT} aria-label="Toggle large text" aria-pressed={largeText} className={`fd-utility-button inline-flex ${largeText ? "border-cobalt bg-[#eef2ff] text-cobalt" : ""}`}>A+</button>
            <button onClick={toggleHC} aria-label="Toggle high contrast" aria-pressed={highContrast} className={`fd-utility-button hidden sm:inline-flex ${highContrast ? "border-cobalt bg-[#eef2ff] text-cobalt" : ""}`}>◐</button>
